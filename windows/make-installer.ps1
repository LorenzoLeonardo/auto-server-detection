...
$ReleaseExe = "$PSScriptRoot\..\target\release\auto-server-detection.exe"
...
`$ExeSource = Join-Path `$InstallerDir "auto-server-detection.exe"
`$DestDir = "`$env:USERPROFILE\bin\auto-server-detection"
`$ExecutableName = "auto-server-detection.exe"
`$ServiceName = "auto-server-detection"
`$NssmPath = "C:\nssm\win64\nssm.exe"
`$NssmDest = "C:\nssm"
...
Write-Host "Copying auto-server-detection.exe to `$DestDir"
